...
void solve(vector<string> &a)
{
    ...
    // find what node leads to rx and then find which nodes lead to that node (in my case, jm and then the 4 nodes leading to jm)
    // do LCM
    ...
    for(int s = 1; s <= 10000000; s++)
    {
        ...
        while(!q.empty())
        {
            ...
            if(input_pulse == 0)
            {
                if(se.find(name) != se.end() && mpp.find(name) != mpp.end() && mpp[name] == 1 && where[name] != s)
                {
                    cnt++;
                    if(ans == 0)
                        ans = s - where[name];
                    ...
                    {
                        long long val = __gcd(ans, s - where[name]);
                        ans = ans * (s - where[name]);
                        ans /= val;
                    }
                    if(cnt == 4)
                    {
